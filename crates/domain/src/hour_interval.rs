...
/// задач (правило "задача готова", см. SPEC.md раздел 3), поэтому привязка к
/// задаче живёт на уровне `TenMinCheck`.
...
mod tests {
    ...
    fn constructs_hour_interval_with_summary() {
        let interval = HourInterval::new(
            HourIntervalId::new(2),
            WorkDayId::new(1),
            UtcTimestamp::new(Utc::now()),
            Some(UtcTimestamp::new(Utc::now())),
            SuccessfulCount::new(5).unwrap(),
            Some("писал код".to_string()),
        );

        assert_eq!(interval.summary(), Some("писал код"));
    }
}
